...
export default function ErrorToast(message: string, options?: IOptionsReactHotToast): void {
  if (!isUseClient()) return;

  if (!isString(message)) {
    console.error("❌ error - ErrorToast - react-hot-toast necesita el mensaje tipo string");
    return;
  }

  if (String(message).trim() === "") {
    console.error(
      "❌ error - ErrorToast - react-hot-toast - el mensaje no puede ser un string vacio ''"
    );
    return;
  }
